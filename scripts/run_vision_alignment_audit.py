...
DEFAULT_TRIGGER_OUT = REPO_ROOT / "reports" / "vision_alignment_audit_trigger_v0.json"
...
def run_audit(
    ...
    out_file: Path,
    mutate: bool = False,
    trigger_out_file: Path | None = None,
) -> dict[str, Any]:
    ...
    report = {
        ...
        "mutation_policy": {
            ...
            "scene_mutation_performed": False,
        },
        "kpi": {
    ...
    if mutate:
        trigger = {
            "trigger_id": f"trigger/vision_alignment_remediation_{date.today().strftime('%Y_%m_%d')}_v0",
            "generated_from": report["artifact_id"],
            "type": "proposed_remediation",
            "actions": [
                {
                    "kind": "backfill_principle_links",
                    "target_artifact_ids": weak_ids,
                }
            ],
            "note": "Generated by audit with --mutate yes; requires explicit human review before applying changes.",
        }
        if trigger_out_file is None:
            trigger_out_file = DEFAULT_TRIGGER_OUT
        trigger_out_file.parent.mkdir(parents=True, exist_ok=True)
        trigger_out_file.write_text(json.dumps(trigger, indent=2) + "\n", encoding="utf-8")
    return report
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        choices=("no", "yes"),
        default="no",
        help="Mutation intent flag. no=propose only (default), yes=emit remediation trigger artifact.",
    )
    ap.add_argument(
        "--trigger-out-file",
        default=str(DEFAULT_TRIGGER_OUT),
        help="Trigger JSON output path used when --mutate yes.",
    )
    args = ap.parse_args()
    ...
    report = run_audit(
        ...
        Path(args.out_file),
        mutate=(args.mutate == "yes"),
        trigger_out_file=Path(args.trigger_out_file),
    )
